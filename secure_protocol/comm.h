 ...
 * other communication mechanisms (shared memory, pipes, etc.) by implementing
 * the same interface.
 */
...
typedef enum { COMM_DEVICE_EOT, COMM_DEVICE_HOT } comm_device_type_t;
 ...
 * @return Length of received message, -1 on timeout, -2 on signature failure,
 * negative for legacy messages
 */
