...
comm_handle_t *comm_init(comm_device_type_t device_type, uint32_t timeout_ms,
  ...
  strncpy(handle->send_addr.sun_path, send_path,
          sizeof(handle->send_addr.sun_path) - 1);
  ...
  if (setsockopt(handle->recv_fd, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv)) <
      0) {
    perror("setsockopt");
    close(handle->send_fd);
    close(handle->recv_fd);
...
ssize_t comm_send_raw(comm_handle_t *handle, const uint8_t *data, size_t len) {
  ...
  ssize_t sent =
      sendto(handle->send_fd, data, len, 0,
             (struct sockaddr *)&handle->send_addr, sizeof(handle->send_addr));
  if (sent < 0) {
    perror("sendto");
    return -1;
  }
