...
int ext_timer_init(void) { return 0; }
...
void ext_timer_sleep_ms(uint32_t ms) { usleep(ms * 1000); }

void ext_timer_init_cycles(void) {}

uint32_t ext_timer_cycles(void) { return 0; }
...
void ext_io_putc(char c) { fputc(c, stdout); }

void ext_io_puts(const char *str) { fputs(str, stdout); }
...
int ext_io_getc(void) { return fgetc(stdin); }
...
void ext_exit(int status) { exit(status); }
...
int ext_random_init(uint32_t seed) {
  if (seed != 0) {
    ext_io_eprintf("Warning: Seeded randomness is not supported on Unix. "
                   "Ignoring seed.\n");
  }
