...
static void vcpu_insn_matched_exec_before(unsigned int cpu_index, void *udata) {
  ...
  MatchCount *match =
      qemu_plugin_scoreboard_find(insn_match->counts, cpu_index);
    ...
    if (matches->len) {
      ...
      for (int j = 0; j < matches->len; j++) {
        ...
        if (g_str_has_prefix(insn_disas, m->match_string)) {
          Instruction *rec =
              get_insn_record(insn_disas, qemu_plugin_insn_vaddr(insn), m);

          qemu_plugin_register_vcpu_insn_exec_cb(
              insn, vcpu_insn_matched_exec_before, QEMU_PLUGIN_CB_NO_REGS, rec);
        }
      }
      g_free(insn_disas);
    }
...
static void plugin_exit(qemu_plugin_id_t id, void *p) {
  ...
  for (i = 0; i < matches->len; ++i) {
    ...
    while (g_hash_table_iter_next(&iter, NULL, (void **)&record)) {
      if (record->match == m) {
        g_string_printf(out, "  %" PRIx64 ": %s (hits %" PRId64 ")\n",
                        record->vaddr, record->disas, record->hits);
        qemu_plugin_outs(out->str);
      }
    }
